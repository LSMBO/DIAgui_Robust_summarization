             ...
             sidebarLayout(
               sidebarPanel(
                 ...
                 conditionalPanel(condition = "input.tab1 == 'Import your data'",
                                  HTML("<p>After your analysis with the DIA-nn software, you can find the files 'report.parquet' and 'report.tsv' in your results.
                                           From one of this file, you can filter your data according to your criterias, use the MaxLFQ algorithm
                                           ...
                                           As the author of DIA-NN recommended it, it is preferable to use the .parquet file. For example, only the
                                           .parquet file contains the 'Peptidoform.Q.Value' column, not the .tsv one, which can be useful for PTM studies
                                           such as phosphoproteomics dataset. <br><br>
...
server <- function(input, output, session){
  ...
  output$reportdata_check <- reactive({
    ...
    if(length(cn)){
      needed_info <- list("Run" = "The name of the analyzed files: KL_Try_001_a",
                          "File.Name" = "The file names used for your analysis: D:/Raw/PXD010529/KL_Try_001_a.mzML.dia",
      ...
      if(all(needed %in% cn)){
        ...
        if("Run" %in% needed & !("File.Name" %in% needed)){
          needed <- needed[-which(needed == "Run")]
        }
        else if("File.Name" %in% needed & !("Run" %in% needed)){
          needed <- needed[-which(needed == "File.Name")]
        }
  ...
  output$frac_dat <-  renderUI({
    if(!is.null(Report_data$d)){
      if("Run" %in% colnames(Report_data$d)){
        fr <- unique(Report_data$d$Run)
      }
      else{
      ...
      if("Run" %in% colnames(report_data())){
        fr <- unique(report_data()$Run)
      }
      else{
  ...
  observeEvent(input$change_dat, {
    ...
    if(input$chorename_dat == "New names"){
        ...
        if(length(unique(nm)) != length(nm)){
          ...
          if(length(change)){
            ...
            for(i in 1:length(change)){
              if("Run" %in% colnames(report)){
                report$Run[which(report$Run == change[i])] <- new[i]
              }
              else{
      ...
      if(input$whattorm_dat == 1){
        ...
        for(i in 1:length(nm)){
          if("Run" %in% colnames(report)){
            report$Run[which(report$Run == cd[i])] <- nm[[i]]
          }
          else{
        ...
        if("Run" %in% colnames(report)){
          report$Run <- str_remove_all(report$Run, ".{1,}\\\\")
        }
        else{
    ...
    precu <- reactive({
      ...
      d <- diann_matrix(df, sample.header = smpl_header,
      ...
      if(input$getPTM_pep){
        if(all(c("Peptidoform.Q.Value", "Global.Peptidoform.Q.Value", "Lib.Peptidoform.Q.Value",
          ...
          ptm <- ptm %>%
            ...
            summarize(Peptidoform.Q.Value = min(Peptidoform.Q.Value),
                      Global.Peptidoform.Q.Value = min(Global.Peptidoform.Q.Value),
                      Lib.Peptidoform.Q.Value = min(Lib.Peptidoform.Q.Value),
                      PTM.Q.Value = min(PTM.Q.Value),
        ...
        else if(all(c("PTM.Q.Value", "PTM.Site.Confidence","Lib.PTM.Site.Confidence") %in% colnames(df))){
          ptm <- df[,c(colnames(d)[1:5],
                      "PTM.Q.Value", "PTM.Site.Confidence",
                      "Lib.PTM.Site.Confidence")]
    ...
    peplfq <- reactive({
      ...
      if(input$wLFQ_peplfq == "diann"){
        d <- diann_maxlfq(df, sample.header = smpl_header,
    ...
    pg <- reactive({
      ...
      if(input$iBAQ_pg){
        ...
        brut <- diann_matrix(brut, sample.header = smpl_header,
    ...
    visu_data <- reactive({
      ...
      if(is.null(Report_data$d) | input$choice_visu == "dat"){
        ...
        if(input$bdata_visu == "Protein group"){
          ...
          df <- pep_ev$x
          if(any(c("Peptidoform.Q.Value", "Global.Peptidoform.Q.Value", "Lib.Peptidoform.Q.Value",
                   "PTM.Q.Value", "PTM.Site.Confidence","Lib.PTM.Site.Confidence") %in% colnames(df))){
            df <- df[,-which(colnames(df) %in% c("Peptidoform.Q.Value", "Global.Peptidoform.Q.Value", "Lib.Peptidoform.Q.Value",
                                                 "PTM.Q.Value", "PTM.Site.Confidence","Lib.PTM.Site.Confidence"))]
          }
        }
    ...
    rtg <- reactive({
      smpl_header <- ifelse("Run" %in% colnames(Report_data$d), "Run", "File.Name")
      g <- ggplot(Report_data$d, aes(iRT, RT, label1 = Precursor.Id, label2 = Protein.Ids,
                                     label3 = Genes, color = PG.Q.Value)) +
        geom_point() +
        facet_wrap(smpl_header) +
        labs(title = "Report data") + theme(plot.title = element_text(hjust = 0.5))
      g
    })
    rtf <- reactive({
      if(!is.null(visu_data())){
        ...
        smpl_header <- ifelse("Run" %in% colnames(d), "Run", "File.Name")
        g <- ggplot(d, aes(iRT, RT, label1 = Precursor.Id, label2 = Protein.Ids,
                           label3 = Genes, color = PG.Q.Value)) +
          geom_point() +
          facet_wrap(smpl_header) +
          labs(title = "Report data filtered") + theme(plot.title = element_text(hjust = 0.5))
        g
      }
      else{
        NULL
    ...
    ptypg <- reactive({
      smpl_header <- ifelse("Run" %in% colnames(Report_data$d), "Run", "File.Name")
      ptyp <- Report_data$d[, c(smpl_header, "Proteotypic")]
    ...
    ptypf <- reactive({
      if(!is.null(visu_data())){
        ...
        smpl_header <- ifelse("Run" %in% colnames(d), "Run", "File.Name")
        ptyp <- d[d[[nm]] %in% visu_data()$id,c(smpl_header, "Proteotypic")]
        ptyp$Proteotypic <- as.character(ptyp$Proteotypic)

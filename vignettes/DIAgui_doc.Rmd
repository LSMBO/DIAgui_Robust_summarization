...
output: 
  rmarkdown::html_document:
    ...
    highlight: espresso
vignette: >
  %\VignetteIndexEntry{Use the DIAguiLSMBO package}
...
The DIAguiLSMBO package is currently on github, so for installing it you'll also need the "devtools" package.
You can install the DIAguiLSMBO package with this commands:
...
You can access the same informations (and the source code) on the [github repository](https://github.com/mgerault/DIAguiLSMBO)
...
2. Activate `DIAguiLSMBO` package by library it.  
...
DIAguiLSMBO contains a small report from DIA-NN, which is also available in [diann-rpcakge](https://github.com/vdemichev/diann-rpackage) from V. Demichev. This report was obtained from proteomics data from the SACCHAROMYCES CEREVISIAE.
...
With this small dataset, you can easily test the functions from DIAguiLSMBO package and also save it as a tsv file to test the shiny application.
...
DIAguiLSMBO contains several functions to check the quality of your data. But first they need to be in the right format.
